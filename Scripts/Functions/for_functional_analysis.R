...
# Obsolete, replaced by run_lm_on_props and run_lmer_on_props in common_functions.R
...
read_table_if_no_rds <- function(name_of_table, rds_file_name, base_dir, pattern = "_1.txt") {
  ...
  if (!file.exists(here("Data",rds_file_name))) {
    name_of_table <- read_gene_results_no_taxon(base_dir, pattern = "_1.txt")
...
# usage:
# > keggGetWrapper("K02775")
# [1] "galactitol PTS system EIIC component"
# > keggGetWrapper("K02787")
# [1] "Not found"
keggGetWrapper = function(x) {
  tryCatch(keggGet(x)[[1]]$DEFINITION, error = function(e) {"Not found"})
}
